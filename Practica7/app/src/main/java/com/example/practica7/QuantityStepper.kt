...
import com.example.practica7.databinding.ViewQuantityStepperBinding
...
class QuantityStepper @JvmOverloads constructor(
    ...
    init{
        ...
        if(attrs != null){
            val ta = context.obtainStyledAttributes(attrs, R.styleable.QuantityStepper)
    ...
    private fun assignEvents(){
        ...
        binding.btnMinus.setOnLogClickListener {
            setQuantity(min, fromUser = true)
            true
        }
        binding.btnPlus.setOnLogClickListener {
    ...
    fun setQuantity(value: Int, fromUser: Boolean = false){
        ...
        if(fromUser && value != clamped){
            Toast.makeText(context, "Limite $min, $max", Toast.LENGTH_LONG).show()
        }
